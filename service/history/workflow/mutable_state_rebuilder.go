...
// during state rebuild (replication / reset). Nexus operations can be backed by EITHER the HSM tree
// or the CHASM tree, so this uses a probe-and-fallback strategy resolved by ScheduledEventId:
//
...
//     HSM tree (legacy behavior). See applyNexusOperationScheduledEvent for the deliberate design
//     rationale (reset realigns an op to current creation policy) and the CHASM->HSM safety fallback.
...
//     workflow tree.
//
// Because creation policy can change between an op's original scheduling and a later reset, an op may
// be reconstructed into the other tree. That is acceptable: everything downstream (non-create event
// apply here, reset reapply, and async completion) resolves the op by ScheduledEventId with the same
// HSM<->CHASM fallback, so resolution stays correct regardless of which tree currently owns the op.
...
// backing tree by the per-namespace nexusoperation.enableChasmWorkflowOperations flag.
//
// Design note (deliberate): unlike non-create events, the create is not "probed" against an existing
// component (none exists yet) — instead it is routed by current creation policy. This means a reset
// realigns an op to whichever framework new ops would be created in today. That can move an op's
// backing rail relative to the original run (e.g. an op originally created in HSM is rebuilt into
// CHASM after the flag is flipped on). This is safe during HSM/CHASM coexistence because both trees
// key ops by ScheduledEventId and all downstream resolution uses the cross-tree fallback.
//
// Rollout resilience: if creating in the CHASM tree errors (e.g. registry/component issues during a
// flag rollout), we fall back to creating the op in the HSM tree rather than failing the whole
// rebuild. The op is still fully resolvable from either tree via ScheduledEventId.
func (b *MutableStateRebuilderImpl) applyNexusOperationScheduledEvent(
	ctx context.Context,
	event *historypb.HistoryEvent,
) error {
	// Gate on CHASM being enabled and wired first (cheap, no namespace lookup); only then read the
	// per-namespace creation-policy flag.
	...
	if err != nil {
		// CHASM create failed; fall back to HSM so a flag-rollout hiccup can't break the rebuild.
		b.logger.Warn(
...
// applyHSMEvent applies an event to the HSM tree, returning the original
// serviceerror.InvalidArgument for unknown event types (preserving prior behavior).
...
func (b *MutableStateRebuilderImpl) applyChasmEvent(
	...
	// Ensure the root CHASM workflow component exists before applying. For the create event
	// (NexusOperationScheduled) on a workflow whose CHASM tree was not yet initialized, this sets the
	// root component; for non-create events the component already exists and this is a no-op.
